...
### 2026-08-25 — צ׳אט רק בתוך האפליקציה (לא במסך כניסה)
- **מה השתנה:** כפתור הצ׳אט הוסר ממסך ההתחברות/אורחים; מוצג רק אחרי התחברות — כולל באפליקציית הבעלים. ניווט עוזר תומך במסכי `/owner/*`.
- **למה:** הצ׳אט שייך לשימוש באפליקציה, לא למסך לוגין.
- **איפה שונה בקוד:** `components/AssistantFab.tsx`, `app/_layout.tsx`, `lib/assistantNav.ts`, `hooks/useAssistantLiveContext.ts`, `server/src/routes/assistant.ts`, `change.md`
- **איפה שונה באפליקציה:** אין FAB ב־`/account`; יש FAB אחרי כניסה (בעלים/מנהל/עובד)

### 2026-08-25 — דמו: כמה סניפים לאותו בעלים
- **מה השתנה:** לבעל הדמו נוצרים 4 סניפים (רוטשילד, דיזנגוף, נמל יפו, הרצליה) — כל אחד עם מלאי/מתכונים/עובדים/סידור. אחרי כניסה נפתח מסך בחירת סניף.
- **למה:** להציג רשת עם כמה סניפים, לא סניף יחיד.
- **איפה שונה בקוד:** `server/src/services/demoSeed.ts`, `change.md`
- **איפה שונה באפליקציה:** אחרי לוגין → hub עם רשימת סניפים

### 2026-08-25 — דמו כשר בשרי: מלאי/מתכונים מורחבים + מחיר וספק
- **מה השתנה:** נתוני דמו גדולים הרבה יותר (עשרות מרכיבים ומתכונים), הכל בשרי או פרווה בלבד (בלי חלבי). במלאי ובתפריט מוצגים בבירור מחיר ליחידה, ספק, וכשרות בקטגוריה; בפירוט מתכון — עלות שורה וספק לכל רכיב. אחרי ריסטארט API נזרעים מחדש אם חסר מרקר המתכון החדש.
- **למה:** הצגה למשקיעים של מטבח כשר בשרי עם מחירים וספקים ברורים.
- **איפה שונה בקוד:** `server/src/services/demoSeed.ts`, `server/src/routes/owner.ts`, `app/owner/inventory.tsx`, `components/owner/OwnerMenuScreen.tsx`, `lib/ownerTypes.ts`, `lib/translations.ts`, `change.md`
- **איפה שונה באפליקציה:** מלאי + תפריט בר/מטבח (מחיר, ספק, פירוט עלות)

### 2026-08-25 — שכר למשמרת והשפעה על רווח
- **מה השתנה:** לכל משמרת מחושב כמה העובד ירוויח (שעתי / חודשי→שעה). בסידור מוצג סכום ליום/שבוע; בבית — שכר מסידור, פירוט לפי עובד, ורווח אחרי שכר (הכנסות − הוצאות − שכר).
- **למה:** בדמו למשקיעים רואים את הקשר בין סידור עבודה להוצאות ולרווח.
- **איפה שונה בקוד:** `server/src/services/laborCost.ts`, `server/src/routes/owner.ts`, `app/owner/schedule.tsx`, `app/owner/index.tsx`, `lib/ownerTypes.ts`, `lib/translations.ts`, `change.md`
- **איפה שונה באפליקציה:** סידור עבודה + בית בעלים (KPI שכר ורווח אחרי שכר)

### 2026-08-25 — דמו למשקיעים: נתונים מלאים + פריסה
- **מה השתנה:** באתחול השרת נזרעים נתוני דמו לבר רוטשילד (עובדים, מלאי, מתכונים, הכנסות/הוצאות, סידור, ספקים). כניסה ממולאת מראש; סניף אחד נכנס ישר לבית; כרטיס ספקים חזר לבית.
- **למה:** אחרי כניסה המסכים לא יהיו ריקים בהצגה למשקיעים.
- **איפה שונה בקוד:** `server/src/services/demoSeed.ts`, `server/src/services/customersService.ts`, `app/account.tsx`, `app/owner/index.tsx`, `app/_layout.tsx`, `lib/translations.ts`, `README.md`, `change.md`
- **איפה שונה באפליקציה:** כניסה → דשבורד עם מספרים וכרטיסים מלאים
